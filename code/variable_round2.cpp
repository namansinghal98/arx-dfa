...
void fault_round_find_setup()
{
	...
	uint64_t count[LOWER_ROUNDS][N][3 * N] = {0};
	...
	for (uint64_t p = 0; p < N; ++p)
	{
		for (uint64_t r = 0; r < LOWER_ROUNDS; ++r)
		{
			for (uint64_t j = 0; j < iterations; ++j) 
			{
				...
				encrypt(x,y);
				faulty_encrypt(xf, yf, ROUNDS, ROUNDS-r, p+1);
	...
	for (uint64_t p = 0; p < N; ++p)
	{
		for (uint64_t i=0;i<LOWER_ROUNDS;i++)
		{
			for (uint64_t j=0;j<3*N;j++)
			{
				B[i][p][j] = 0.5 - (double)count[i][p][j] / (double)iterations;	
			}
		}
	}
...
void fault_round_find_test_complete()
{
	...
	double trail[3 * N] = {0};
	std::vector<std::pair<double, int>> corr(N * LOWER_ROUNDS);
	...
	for (uint64_t l = 0; l < iterations; ++l)
	{
		...
		xf = a;
		yf = b;
		uint64_t r = uni_dist(rng) % (LOWER_ROUNDS);
		...
		{				
			...
			{
				std::pair<double, int>pa = {correlation(trail, B[i][j]), i};
				corr[i*LOWER_ROUNDS + j] = pa;
			}
		}
		...
		counter[r][corr[corr.size() - 1].second]++;
